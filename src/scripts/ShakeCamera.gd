...
@export_range(0.01, 20.0) var vibration_ampl := 1.0
@export var smoothed := false
@export_range(1.0, 10.0) var smooth_speed := 1.0
	...
	if remaining_time_in_s > 0:
		...
		if smoothed:
			if Utils.compare_floats(smooth_offset_x, offset.x, 0.01):
				smooth_offset_x = randf_range(-1, 1) * vibration_ampl
			if Utils.compare_floats(smooth_offset_y, offset.y, 0.01):
				smooth_offset_y = randf_range(-1, 1) * vibration_ampl
			...
			offset.x = randf_range(-1, 1) * vibration_ampl
			offset.y = randf_range(-1, 1) * vibration_ampl
